...
class Operation_Page(CTkFrame):
    def __init__(self, root):
        ...
        entry.grid(row=0, column=0, columnspan=3, sticky="ew", padx=100, pady=(100,50), ipadx=50, ipady=50)


        label_result = CTkLabel(self, text=" . . . ", height=120, font=("Helvetica", 80),
                                corner_radius=50, text_color="#370d40", fg_color="#6D8EA0")
        label_result.grid(row=1, column=0,columnspan=2, sticky="ew", padx=(110,10),  ipadx=50, ipady=5)

        button_calc = CTkButton(master=self, text="Calculate", height=120, fg_color="#6D8EA0",
                                text_color="#370d40", font=("Helvetica", 40), corner_radius=50, command=calc)
        button_calc.grid(row=1, column=2, sticky="ew", padx=(10,110),  ipadx=50, ipady=5)

        bottom = CTkFrame(self, fg_color="transparent")
        bottom.grid(row=2, column=0, columnspan=3, sticky="es", padx=(0, 100), pady=(0, 50))
